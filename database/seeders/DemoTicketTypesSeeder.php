...
class DemoTicketTypesSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $this->seedCatalogTicketTypes();
    }

    private function seedCatalogTicketTypes(): void
    {
        $events = Event::query()->withCount('ticketTypes')->get();
        ...
        foreach ($events as $event) {
            ...
            $priceBase = 2900 + ((crc32((string) $event->seo_slug) % 7) * 1000);
            $hasFreeTier = (crc32((string) $event->seo_slug) % 5) === 0;

            if ($hasFreeTier) {
                TicketType::query()->firstOrCreate(
                    ['event_id' => $event->id, 'name' => 'Community Pass'],
                    [
                        'is_free' => true,
                        'price_amount' => 0,
                        'currency' => 'USD',
                ...
                TicketType::query()->firstOrCreate(
                    ['event_id' => $event->id, 'name' => 'Supporter Pass'],
                    [
                        'is_free' => false,
                        'price_amount' => $priceBase,
                        'currency' => 'USD',
                        'max_per_order' => 4,
                    ]
                );
