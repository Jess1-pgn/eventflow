<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-900">{{ __('Edit Event') }}</h2>
    ...
    @php
        $selectedTimezone = old('timezone', $event->timezone);
        $startsAtValue = old('starts_at', $event->starts_at?->setTimezone($selectedTimezone)->format('Y-m-d\TH:i'));
        $endsAtValue = old('ends_at', $event->ends_at?->setTimezone($selectedTimezone)->format('Y-m-d\TH:i'));
        $selectedCategoryIds = collect(old('category_ids', $event->categories->pluck('id')->all()))->map(fn ($id) => (int) $id)->all();
        $selectedTagIds = collect(old('tag_ids', $event->tags->pluck('id')->all()))->map(fn ($id) => (int) $id)->all();
    @endphp
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="card p-6 md:p-8">
                @if (session('status'))
                    <div class="mb-6 rounded-xl bg-green-50 border border-green-100 px-4 py-3 flex items-center gap-3">
                ...
                <form method="POST" action="{{ route('dashboard.events.update', $event) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <x-text-input id="seo_slug" name="seo_slug" type="text" class="mt-1 block w-full" :value="old('seo_slug', $event->seo_slug)" />
                        <x-input-error :messages="$errors->get('seo_slug')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="description_html" :value="__('Description')" />
                        <textarea id="description_html" name="description_html" rows="5" class="form-input-modern mt-1 block w-full">{{ old('description_html', $event->description_html) }}</textarea>
                    ...
                    <div>
                        ...
                        <select id="venue_id" name="venue_id" class="form-input-modern mt-1 block w-full">
                            ...
                            @foreach ($venues as $venue)
                                <option value="{{ $venue->id }}" @selected((int) old('venue_id', $event->venue_id) === $venue->id)>{{ $venue->name }}</option>
                            @endforeach
                        </select>
                        ...
                        <div>
                            ...
                            <select id="category_ids" name="category_ids[]" multiple class="form-input-modern mt-1 block w-full">
                                @foreach ($categories as $category)
                                    <option value="{{ $category->id }}" @selected(in_array($category->id, $selectedCategoryIds, true))>{{ $category->name }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="starts_at" name="starts_at" type="datetime-local" class="mt-1 block w-full" :value="$startsAtValue" required />
                            <x-input-error :messages="$errors->get('starts_at')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="ends_at" :value="__('Ends At')" />
                            <x-text-input id="ends_at" name="ends_at" type="datetime-local" class="mt-1 block w-full" :value="$endsAtValue" required />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="meta_title" name="meta_title" type="text" class="mt-1 block w-full" :value="old('meta_title', $event->meta_title)" />
                            <x-input-error :messages="$errors->get('meta_title')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="meta_description" :value="__('Meta Description')" />
                            <x-text-input id="meta_description" name="meta_description" type="text" class="mt-1 block w-full" :value="old('meta_description', $event->meta_description)" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="cover_image_path" name="cover_image_path" type="text" class="mt-1 block w-full" :value="old('cover_image_path', $event->cover_image_path)" />
                        ...
                        <div>
                            ...
                            <x-text-input id="banner_image_path" name="banner_image_path" type="text" class="mt-1 block w-full" :value="old('banner_image_path', $event->banner_image_path)" />
                    ...
                    <div class="flex items-center gap-4 pt-4 border-t border-gray-100">
                        ...
                        <a href="{{ route('dashboard.events.index') }}" class="text-sm font-medium text-gray-500 hover:text-gray-700 transition">{{ __('Back to list') }}</a>
                    </div>
                </form>
            </div>
        </div>
