...
new class extends Component
{
    ...
    {
        if (! auth()->check()) {
            return route('public.events.index');
        }
...
<nav x-data="{ open: false }" class="bg-gradient-to-r from-indigo-700 via-violet-700 to-purple-700 shadow-lg">
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                @else
                    <div class="flex items-center gap-2">
                        <a href="{{ route('login') }}" class="inline-flex items-center rounded-lg px-3 py-2 text-sm font-medium text-indigo-100 hover:text-white hover:bg-white/10 transition" wire:navigate>
                            Log in
                        </a>
                        <a href="{{ route('register') }}" class="inline-flex items-center rounded-lg bg-white text-indigo-700 px-3 py-2 text-sm font-semibold hover:bg-indigo-50 transition" wire:navigate>
                            Register
                        </a>
                    </div>
                @endauth
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden bg-white border-t border-gray-100 shadow-lg">
        ...
        @else
            <div class="pt-4 pb-3 border-t border-gray-100 px-4 space-y-2">
